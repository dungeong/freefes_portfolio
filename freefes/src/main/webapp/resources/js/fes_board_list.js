...
function update_paging(paging_data, query_string) {
	...
	if (paging_data != null) {
		...
		if (paging_data.prev && paging_data.page_begin > 1) {
			fes_board_paging.append(`<a class="paging_number" href="${cpath}/fes_board/` + fes_page_href + `?` + query_string + `${paging_data.page_begin - 1}">◀이전</a>`);
		}
		
		for (let page_number = paging_data.page_begin; page_number <= paging_data.page_end; page_number++) {
			fes_board_paging.append(`<a class="paging_number" href="${cpath}/fes_board/` + fes_page_href + `?` + query_string + `${page_number}">[${page_number}]</a>`);
		}
		
		if (paging_data.next && paging_data.page_end < paging_data.total_count) {
			fes_board_paging.append(`<a class="paging_number" href="${cpath}/fes_board/` + fes_page_href + `?` + query_string + `${paging_data.page_end + 1}">다음▶</a>`);
		}
	}
	else {
		fes_board_paging.empty();
